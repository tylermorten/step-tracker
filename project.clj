(defproject step-tracker "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "step-tracker.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.5.0"]]}
             :uberjar {:aot [step-tracker.server]}}
  :main ^{:skip-aot true} step-tracker.server)
